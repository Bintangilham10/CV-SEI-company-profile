...
export default function Navbar() {
    ...
    const headerClass = `z-50 transition-all duration-300 ${isDarkPage
        ? scrolled || menuOpen
            ? "fixed top-0 left-0 w-full bg-white/95 backdrop-blur-md border-b border-slate-100 shadow-sm py-2"
            : "absolute top-0 left-0 w-full bg-transparent border-b border-transparent py-4"
        : "sticky top-0 bg-white/95 backdrop-blur-md border-b border-slate-100 py-2.5 shadow-sm"
        }`;
    ...
    return (
        <header className={headerClass}>
            <div className="max-w-7xl mx-auto px-6 h-16 md:h-18 flex items-center justify-between">
                ...
                <nav className={`hidden lg:flex items-center gap-8 lg:gap-10 font-medium transition-colors duration-300 ${textClass}`}>
                    {NAV_LINKS.map((l) =>
                        l.label === "Layanan" ? (
                            <div key={l.label} className="relative group">
                                <NavLink
                                    to={l.href}
                                    className={({ isActive }) =>
                                        `transition-colors inline-flex items-center gap-1.5 py-2.5 text-sm md:text-base font-semibold ${hoverClass} ${isActive || isServicePage ? activeTextClass : ""
                                        }`
                                    }
                                >
                                    {l.label}
                                    <svg className="w-4 h-4 transition-transform duration-200 group-hover:rotate-180" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                                    ...
                                    <div className="w-80 bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden py-2">
                                        {SERVICE_DROPDOWN.map(({ label, href, icon: Icon }) => (
                                            <Link
                                                key={label}
                                                to={href}
                                                className={`flex items-center gap-3 px-4 py-3 text-sm transition-colors group/item ${location.pathname === href
                                                        ? "bg-amber-50 text-[#0E2A54] font-semibold"
                                                        : "text-slate-700 hover:bg-amber-50 hover:text-[#0E2A54]"
                                                    }`}
                                            >
                                                <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 transition-colors ${location.pathname === href
                            ...
                            <NavLink
                                ...
                                to={l.href}
                                className={({ isActive }) =>
                                    `transition-colors py-2.5 text-sm md:text-base font-semibold ${hoverClass} ${isActive ? activeTextClass : ""
                                    }`
                                }
                            >
                                {l.label}
                        ...
                        l.label === "Layanan" ? (
                            <div key={l.label}>
                                <button
                                    ...
                                    className={`w-full flex items-center justify-between py-3 transition-colors ${isServicePage ? "text-[#0E2A54] font-bold" : "text-slate-600"
                                        }`}
                                >
                                    <span>{l.label}</span>
                                    <ChevronDown
                                    ...
                                    <div className="pl-3 pb-2 space-y-0.5 border-l-2 border-amber-400 ml-2">
                                        {SERVICE_DROPDOWN.map(({ label, href, icon: Icon }) => (
                                            <Link
                                                ...
                                                className={`flex items-center gap-2.5 py-2.5 px-3 rounded-lg text-xs transition-colors ${location.pathname === href
